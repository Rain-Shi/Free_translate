...
class NoRerunEditInterface:
    """无重新运行编辑界面 - 使用回调函数完全避免跳回"""
    ...
    def display_no_rerun_edit_interface(self):
        """显示无重新运行编辑界面"""
        ...
        # 使用expander来避免重新运行
        with st.expander("📝 编辑界面", expanded=True):
            # 一次性显示所有段落的左右编辑界面
            ...
            if max_paragraphs > 0:
                ...
                for i in range(max_paragraphs):
                    self._display_paragraph_edit_no_rerun(i)
        
        # 操作按钮区域 - 使用expander包装
        with st.expander("🔧 操作按钮", expanded=True):
            col1, col2, col3 = st.columns(3)
    ...
    def _display_paragraph_edit_no_rerun(self, para_index: int):
        """显示单个段落的无重新运行编辑界面"""
        ...
        with col2:
            ...
            edited_text = st.text_area(
                ...
                help="您可以在这里编辑译文内容",
                on_change=self._update_paragraph_text,
                args=(para_index,)
            )
    ...
    def _update_paragraph_text(self, para_index: int):
        """更新段落文本的回调函数"""
        edit_key = f"edited_text_{para_index}"
        if edit_key in st.session_state:
            self.edited_paragraphs[para_index] = st.session_state[edit_key]
