...
class StableEditInterface:
    ...
    def display_stable_edit_interface(self):
        """显示稳定的编辑界面"""
        ...
        with st.form("edit_form"):
            ...
            if max_paragraphs > 0:
                ...
                for i in range(max_paragraphs):
                    self._display_paragraph_edit_form(i)
            
            # 表单提交按钮
            col1, col2, col3 = st.columns(3)
            
            with col1:
                reset_original = st.form_submit_button("🔄 重置所有为原文")
            
            with col2:
                reset_translated = st.form_submit_button("🔄 重置所有为译文")
            
            with col3:
                copy_original = st.form_submit_button("📋 复制所有原文")
    ...
    def _display_paragraph_edit_form(self, para_index: int):
        """显示单个段落的编辑表单"""
